    ...
    while not main_menu_exit:
        ...
        if main_sel == 0:
            ...
            while not user_menu_exit:
                ...
                if user_sel == 1:
                    user_array = users.list_iam_users()
                    users_menu = TerminalMenu(user_array)
                    user_selection = users_menu.show()
                    if user_selection is not None:
                        user_selection_list = [
                            f"List Policies attached to {user_array[user_selection]}",
                            f"List Groups {user_array[user_selection]} is in",
                            f"Add {user_array[user_selection]} to IAM Groups",
                            f"Remove {user_array[user_selection]} from a Group",
                            f"Add Policies to {user_array[user_selection]}",
                            f"Remove Policies from {user_array[user_selection]}",
                            f"Change Password for {user_array[user_selection]}",
                            f"List credentials for {user_array[user_selection]}",
                            f"Revoke credentials for {user_array[user_selection]}",
                            f"Rotate access keys for {user_array[user_selection]}",
                            f"Delete {user_array[user_selection]}",
                            "Return to IAM User Menu"
                        ]

                        user_selection_sub_menu = TerminalMenu(user_selection_list)
                        interact_user_sel = user_selection_sub_menu.show()
                        if interact_user_sel:
                            if interact_user_sel == 0:
                                list_of_managed_policies = users.list_attached_managed_user_policies(username=user_array[user_selection])
                                ...
                                list_of_inline_policies = users.list_attached_inline_user_policies(username=user_array[user_selection])
                                ...
                                list_of_groups = users.list_groups_for_user(user_array[user_selection])
                                ...
                                print(f"Removing {user_array[user_selection]} from groups....")
                                # check groups user is in
                                groups_for_user = users.list_groups_for_user(user_array[user_selection])
                                if isinstance(groups_for_user, str):
                                    print(groups_for_user)
                                elif groups_for_user:
                                    for group in groups_for_user:
                                        print(f"Removing {user_array[user_selection]} from {group}")
                                ...
                                if attach_choice.lower() == "attach":
                                    ...
                                    if arn not in valid_arns:
                                        ...
                                        is_attached_already = users.list_attached_managed_user_policies(
                                            username=user_array[user_selection])
                                        ...
                                        if policy_name in is_attached_already:
                                            ...
                                            attach_attempt = users.attach_user_policy(username=user_array[user_selection], policy_arn=arn)
                                            print(attach_attempt)
                                    ...
                                    if created_policy:
                                        policy_arn = created_policy  # ARN returned by create_policy
                                        print(f"Attaching {policy_arn}...")
                                        attach_response = users.attach_user_policy(username=user_array[user_selection],
                                                                                   policy_arn=policy_arn)
                                        print(attach_response)
                                    else:
                                        print("Policy creation failed; cannot attach.")
                                ...
                                if all_policies.lower() == "all":
                                    ...
                                    list_of_managed_policies = users.list_attached_managed_user_policies(user_array[user_selection])
                                    if isinstance(list_of_managed_policies, str):
                                        print(list_of_managed_policies)
                                    elif list_of_managed_policies:
                                        for policy in list_of_managed_policies:
                                            ...
                                            users.detach_user_policy(username=user_array[user_selection], policy_arn=policy_arn)
                                            print(f"Detached {policy_arn} from {user_array[user_selection]}")
                                    ...
                                    if isinstance(list_of_inline_policies, str):
                                        ...
                                        for policy in list_of_inline_policies:
                                            ...
                                            delete_inline_policies = users.delete_user_policy(username=user_array[user_selection], policy_name=policy)
                                    ...
                                    list_of_policies_attached = users.list_attached_managed_user_policies(user_array[user_selection]) # returns non arns
                                    for policy in list_of_policies_attached:
                                        policy_arn = iam_policy.get_iam_policy_arn(new_policy=policy)
                                        if policy_arn == specific_arn:
                                            detach_result = users.detach_user_policy(username=user_array[user_selection], policy_arn=policy_arn)
                                            print(detach_result)
                                ...
                                change_password = users.change_password(user_array[user_selection], password)
                                ...
                                if creds_choice.lower() not in creds_options:
                                    ...
                                    access_keys = users.list_access_keys(user_array[user_selection])
                                    ...
                                    certs_list = users.list_certificate_ids(user_array[user_selection])
                                    ...
                                    ssh_key_list = users.list_public_ssh_keys(user_array[user_selection])
                                    ...
                                    service_cred_list = users.list_service_specific_creds(user_array[user_selection])
                                    ...
                                    mfa_devices_list = users.list_mfa_devices(user_array[user_selection])
                                ...
                                if credential_choice not in creds_options:
                                    ...
                                    list_of_access_keys = users.list_access_keys(user_array[user_selection])
                                    if isinstance(list_of_access_keys, str):
                                        print(list_of_access_keys)
                                    elif list_of_access_keys:
                                        for key in list_of_access_keys:
                                            print(f"Deleting key: {key}")
                                            key_deletion = users.delete_access_key(username=user_array[user_selection], access_key_id=key)
                                            print(key_deletion)


                                elif credential_choice == "certificate":
                                    print("revoking certificates")
                                    list_of_certificates = users.list_certificate_ids(user_array[user_selection])
                                    if isinstance(list_of_certificates, str):
                                        ...
                                        for cert in list_of_certificates:
                                            ...
                                            cert_deletion = users.delete_signing_certificate(username=user_array[user_selection], cert=cert)
                                            print(cert_deletion)

                                elif credential_choice == "public ssh key":
                                    print("revoking public ssh keys")
                                    list_of_ssh_keys = users.list_public_ssh_keys(user_array[user_selection])
                                    if isinstance(list_of_ssh_keys, str):
                                        ...
                                        for key in list_of_ssh_keys:
                                            ...
                                            key_deletion = users.delete_ssh_public_key(username=user_array[user_selection], key_id=key)
                                            print(key_deletion)
                                elif credential_choice == "service credentials":
                                    print("Revoking Service Credentials..")
                                    list_of_creds = users.list_service_specific_creds(user_array[user_selection])
                                    if isinstance(list_of_creds, str):
                                        print(list_of_creds)
                                    elif list_of_creds:
                                        for cred in list_of_creds:
                                            cred_deletion = users.delete_service_specific_creds(user_array[user_selection], cred=cred)
                                            print(cred_deletion)
                                elif credential_choice == "mfa devices":
                                    print("Deactivating MFA devices..")
                                    list_of_mfa = users.list_mfa_devices(user_array[user_selection])
                                    if isinstance(list_of_mfa, str):
                                        print(list_of_mfa)
                                    elif list_of_mfa:
                                        for device in list_of_mfa:
                                            device_deactivation = users.deactivate_mfa_device(user_array[user_selection], serial_id=device)
                                ...
                                if isinstance(list_of_access_keys, str):
                                    ...
                                    if which_key_rotate not in range(0, len(length_list) -1):
                                        ...
                                        if revoke != "yes":
                                            ...
                                            revoke_key = users.delete_access_key(user_array[user_selection], access_key_id=chosen_key)
                                            print(revoke_key)
                                    # create new key and expose access key id and secret access
                                            ...
                                            if create != "yes":
                                                ...
                                                new_access_key_id, new_secret_access_key = users.create_access_key(
                                                    user_array[user_selection])
                                                print(f"Access Key ID: {new_access_key_id[0]}")
                                                print(f"Secret Access Key: {new_secret_access_key[0]}")


                            elif interact_user_sel == 10:
                                print(f"Deleting {user_array[user_selection]}...\n")
                                iam_user_delete_response = users.delete_iam_user(user_array[user_selection])
